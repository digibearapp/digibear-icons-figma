...
const { prefixName, toCamelCase } = require("./utils");

function generateIconsMetadata() {
  let rawIconsMetadata = JSON.parse(fs.readFileSync(paths.ICONS_METADATA_JSON_PATH));
  const iconsMetadata = Object.keys(rawIconsMetadata).sort().reduce(
    (obj, key) => { 
...
function createMetadataFile(fileLines) {
  try {
    ...
    console.error(
      `${chalk.inverse.red(" FAIL ")} Failed to created digibear-icons-metadata.ts.`
    );
    console.group();
    console.error(err);
    console.groupEnd();
...
module.exports = { generateIconsMetadata }